...
def backward(Z1,A1,Z2,A2,Z3,A3, input, output):
    ...
    for j in range (32):
        for k in range (10):
            dC0_daj2[j]+= 2*(A3[k]-output[k])*dSigmoid(Z3[k])*iW3[j][k]
    daj2_dzj2 = np.zeros(32)
    for j in range (32):
        daj2_dzj2[j] = dSigmoid(Z2[j])
    ...
    for i in range (32):
        for j in range (32):
            dC0_dai1[i]+=dC0_daj2[j] * dSigmoid(Z2[j]) * iW2[i][j]
    dai1_dzi1 = np.zeros(32)
    for i in range (32):
        dai1_dzi1[i] = dSigmoid(Z1[i])
...
def init_GD(): #print the cost function
    ...
    for i in range (1,epoch+1):
        ...
        input/=255.0
        Z1, A1, Z2, A2, Z3, A3 = forward(iW1,iB1,iW2,iB2,iW3,iB3, input)
